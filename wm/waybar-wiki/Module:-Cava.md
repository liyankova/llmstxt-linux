...
| `ascii_max_range`            | int  |    7           | Impossible to set directly. The value is dictated by the number of icons in the array `format-icons`|
| `data_format`            | string  |    asci           | Impossible to set. Waybar sets it to = asci for internal needs |
| `raw_target`            | string  |    /dev/stdout           | Impossible to set. Waybar sets it to = /dev/stdout for internal needs |
