...
like `MANGOCONFIG=/home/xxx/mango`
...
| `cursor_size` | integer | 24 |(0 - 9999) Set cursor size (need relogin to apply) |
| `cursor_theme` | string | - | Set cursor theme (needs a relogin to apply) |
...
| `isglobal` | integer | 0,1 | Open as global winodw|
...
yay -S wlr-randr
```
- you can use `wlr-dpms` to toggle monitor off or on.
```bash
yay -S wlr-dpms
```

- you can also use mmsg to trun on/off monitor
```bash
