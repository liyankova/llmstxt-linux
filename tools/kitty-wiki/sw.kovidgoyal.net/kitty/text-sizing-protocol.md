---
source_url: "https://sw.kovidgoyal.net/kitty/text-sizing-protocol/"
title: "The text sizing protocol - kitty"
crawl_date: "2025-11-16T14:50:21.004237Z"
selector: "article"
---

# The text sizing protocol - kitty
...
To detect support for this protocol use the [CPR (Cursor Position Report)](https://vt100.net/docs/vt510-rm/CPR.html) escape code. Send a `CR`
(carriage return) followed by `CPR` followed by `\e]66;w=2; \a`
which will draw a space character in two cells, followed by another `CPR`.
Then send `\e]66;s=2; \a` which will draw a space in a `2 by 2`
...
[Grapheme segmentation algorithm](https://www.unicode.org/reports/tr29/#Grapheme_Cluster_Boundaries)
...
[maximal subpart of the ill-formed subsequence](https://www.unicode.org/versions/Unicode16.0.0/core-spec/chapter-3/#G66453)
...
2. Next, check if the code point is *invalid*, and if it is, discard it
   and finish processing. Invalid code points are code points with Unicode category `Cc or Cs`
   and 66 additional code points: `[0xfdd0, 0xfdef]`, `[0xfffe, 0x10ffff-1, 0x10000]`
   and `[0xffff, 0x10ffff, 0x10000]`.
...
6. If there is a previous cell, the
   [Grapheme segmentation algorithm UAX29-C1-1](https://www.unicode.org/reports/tr29/#C1-1)
...
7. If there is no boundary, the current code point is added to the previous
   cell and processing of the code point is finished. See the [Unicode variation selectors](#var-select)
...
Notation: `[start, stop, step]` means the integers from `start`
to `stop` in increments of `step`. When the step is not
...
2. *Doublewidth*: Parse [EastAsianWidth.txt](https://www.unicode.org/Public/UCD/latest/ucd/EastAsianWidth.txt) from
   ...
   they are marked as `A` in `EastAsianWidth.txt`: `[0x3400,
...
3. *Wide emoji*: Parse [emoji-sequences.txt](https://www.unicode.org/Public/emoji/latest/emoji-sequences.txt) from
   ...
   followed by `FE0F` in the file. The leading copdepoints in all
   `RGI_Emoji_Modifier_Sequence` and `RGI_Emoji_Tag_Sequence` have width two.
...
4. *Marks*: These are all zero width code points. They are code points with Unicode
   ...
   all modifier code points from `RGI_Emoji_Modifier_Sequence` in the
