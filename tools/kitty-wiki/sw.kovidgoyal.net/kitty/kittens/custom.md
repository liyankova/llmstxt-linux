---
source_url: "https://sw.kovidgoyal.net/kitty/kittens/custom/"
title: "Custom kittens - kitty"
crawl_date: "2025-11-16T14:48:41.660146Z"
selector: "article"
---

# Custom kittens - kitty

# Custom kittens[¶](#custom-kittens "Link to this heading")
...
window/scrollback to the kitten over its `STDIN`. The kitten can then
...
(you might need to adjust the path to wherever the [kitty config directory](../../conf/#confloc) is on your machine).
...
Start kitty and press `Ctrl`+`K` and you should see the kitten running.
The best way to develop your own kittens is to modify one of the built-in
kittens. Look in the [kittens sub-directory](https://github.com/kovidgoyal/kitty/tree/master/kittens) of the kitty source
code for those. Or see below for a list of [third-party kittens](#external-kittens), that other kitty users have created.

## kitty API to use with kittens[¶](#kitty-api-to-use-with-kittens "Link to this heading")
...
[Remote control API](../../remote-control/). Simply call
`boss.call_remote_control()`, with the same arguments you
...
Inside handle\_result() the active window is still the window in which the
kitten was run, therefore when using call\_remote\_control() be sure to pass
...
## Passing arguments to kittens[¶](#passing-arguments-to-kittens "Link to this heading")
...
## Passing the contents of the screen to the kitten[¶](#passing-the-contents-of-the-screen-to-the-kitten "Link to this heading")
...
`STDIN`. There are many other types of input you can ask for, described in
the table below:

Types of input to kittens[¶](#id1 "Link to this table")

| Keyword | Type of `STDIN` input |
| --- | --- |
...
For the types based on the output of a command, [Shell integration](../../shell-integration/#shell-integration) is
required.

## Using kittens to script kitty, without any terminal UI[¶](#using-kittens-to-script-kitty-without-any-terminal-ui "Link to this heading")
...
equivalent to the builtin [`toggle_layout`](../../actions/#action-toggle_layout) action.

Create a Python file in the [kitty config directory](../../conf/#confloc),
...
Pressing `F11` will now act as a zoom toggle function. You can get even
...
## Sending mouse events[¶](#sending-mouse-events "Link to this heading")
...
## Using remote control inside the main() kitten function[¶](#using-remote-control-inside-the-main-kitten-function "Link to this heading")
...
`allow_remote_control=True` tells kitty to run this kitten with remote
control enabled, regardless of whether it is enabled globally or not.
To run a remote control command use the `main.remote_control()` function
which is a thin wrapper around Python’s `subprocess.run` function. Note
that by default, for security, child processes launched by your kitten cannot use remote
control, thus it is necessary to use `main.remote_control()`. If you wish
...
[`remote_control_password`](../../conf/#opt-kitty.remote_control_password) for details. The password value is accessible
as `main.password` and is used by `main.remote_control()`
automatically.

## Debugging kittens[¶](#debugging-kittens "Link to this heading")
...
## Developing builtin kittens for inclusion with kitty[¶](#developing-builtin-kittens-for-inclusion-with-kitty "Link to this heading")

There is documentation for [Developing builtin kittens](../developing-builtin-kittens/) which are written in the Go
language.

## Kittens created by kitty users[¶](#kittens-created-by-kitty-users "Link to this heading")

[vim-kitty-navigator](https://github.com/knubie/vim-kitty-navigator)
:   Allows you to navigate seamlessly between vim and kitty splits using a
    consistent set of hotkeys.

[smart-scroll](https://github.com/yurikhan/kitty-smart-scroll)
:   Makes the kitty scroll bindings work in full screen applications

[kitty-tab-switcher](https://github.com/OsiPog/kitty-tab-switcher)
...
[insert password](https://github.com/kovidgoyal/kitty/issues/1222)
