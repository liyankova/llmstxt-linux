...
title: "Desktop notifications - kitty"
crawl_date: "2025-11-16T14:50:30.922236Z"
selector: "article"
---

# Desktop notifications - kitty

# Desktop notifications[¶](#desktop-notifications "Link to this heading")
...
defunct [terminal-wg](https://gitlab.freedesktop.org/terminal-wg/specifications/-/issues/13)
...
Here `<OSC>` is `<ESC>]` and `<terminator>` is
`<ESC><backslash>`. The `metadata` is a section of colon separated
`key=value` pairs. Every key must be a single character from the set
`a-zA-Z` and every value must be a word consisting of characters from
the set `` a-zA-Z0-9-_/+.,(){}[]*&^%$#@!`~ ``. The payload must be
...
*kitty* also comes with its own [statically compiled command line tool](../kittens/notify/) to easily display
...
key is the *notification id* which is an [Identifier](#identifier).
...
Both the `title` and `body` payloads must be either [Escape code safe UTF-8](#safe-utf8) text
or UTF-8 text that is [Base64](#base64) encoded, in which case there must be an
`e=1` key in the metadata to indicate the payload is [Base64](#base64)
encoded. No HTML or other markup in the plain text is allowed. It is strictly
plain text, to be interpreted as such.

## Allowing users to filter notifications[¶](#allowing-users-to-filter-notifications "Link to this heading")

Added in version 0.36.0: Specifying application name and notification type
...
notifications they do not want. Both keys must have [Base64](#base64)
...
applications [desktop file](https://specifications.freedesktop.org/desktop-entry-spec/desktop-entry-spec-latest.html#file-naming)
...
capabilities via [`filter_notification`](../conf/#opt-kitty.filter_notification).

## Being informed when user activates the notification[¶](#being-informed-when-user-activates-the-notification "Link to this heading")
...
Complete specification of all the metadata keys is in the [table below](#keys-in-notificatons-protocol).
...
## Being informed when a notification is closed[¶](#being-informed-when-a-notification-is-closed "Link to this heading")

Added in version 0.36.0: Notifications of close events
...
## Updating or closing an existing notification[¶](#updating-or-closing-an-existing-notification "Link to this heading")

Added in version 0.36.0: The ability to update and close a previous notification
...
## Automatically expiring notifications[¶](#automatically-expiring-notifications "Link to this heading")
...
## Adding icons to notifications[¶](#adding-icons-to-notifications "Link to this heading")

Added in version 0.36.0: Custom icons in notifications
...
key. The value of this key is [Base64](#base64) encoded UTF-8 text. Names
...
Universally available icon names[¶](#id3 "Link to this table")
...
such as the filename of the application’s `.desktop` file on Linux or its
bundle identifier on macOS. For example if the cross-platform application
FooBar has a desktop file named: `foo-bar.desktop` and a bundle
...
### Adding icons by transmitting icon data[¶](#adding-icons-by-transmitting-icon-data "Link to this heading")
...
key must be a random globally unique UUID like [Identifier](#identifier). Then, the
...
## Adding buttons to the notification[¶](#adding-buttons-to-the-notification "Link to this heading")
...
sent either as [Escape code safe UTF-8](#safe-utf8) or [Base64](#base64). When the user clicks on one
...
Here, button\_number is a number from 1 onwards, where 1 corresponds
...
## Playing a sound with notifications[¶](#playing-a-sound-with-notifications "Link to this heading")

Added in version 0.36.0: The ability to control the sound played with notifications
...
This can be done by using the `s` key which accepts [Base64](#base64) encoded
...
probably support the [standard sound names](https://specifications.freedesktop.org/sound-naming-spec/latest/#names)

Standard sound names[¶](#id4 "Link to this table")
...
## Querying for support[¶](#querying-for-support "Link to this heading")

Added in version 0.36.0: The ability to query for support
...
## Specification of all keys used in the protocol[¶](#specification-of-all-keys-used-in-the-protocol "Link to this heading")
...
| `e` | `0` or `1` | `0` | If set to `1` means the payload is [Base64](#base64) encoded UTF-8, otherwise it is plain UTF-8 text with no C0 control codes in it |
| `f` | [Base64](#base64) encoded UTF-8 application name | `unset` | The name of the application sending the notification. Can be used to filter out notifications. |
| `g` | [Identifier](#identifier) | `unset` | Identifier for icon data. Make these globally unique, like an UUID. |
| `i` | [Identifier](#identifier) | `unset` | Identifier for the notification. Make these globally unique, like an UUID, so that terminal multiplexers can direct responses to the correct window. Note that for backwards compatibility reasons i=0 is special and should not be used. |
| `n` | [Base64](#base64) encoded UTF-8 application name | `unset` | Icon name. Can be specified multiple times. |
...
| `s` | [Base64](#base64) encoded sound name | `system` | The sound name to play with the notification. `silent` means no sound. `system` means to play the default sound, if any, of the platform notification service. Other names are implementation dependent. |
| `t` | [Base64](#base64) encoded UTF-8 notification type | `unset` | The type of the notification. Used to filter out notifications. Can be specified multiple times. |
...
Added in version 0.35.0: Support for the `u` key to specify urgency

Added in version 0.31.0: Support for the `o` key to prevent notifications from focused windows

Note

*kitty* also supports the [legacy OSC 9 protocol developed by iTerm2](https://iterm2.com/documentation-escape-codes.html) for desktop
notifications.

## Base64[¶](#base64 "Link to this heading")

The base64 encoding used in the this specification is the one defined in
[**RFC 4648**](https://datatracker.ietf.org/doc/html/rfc4648.html). When a base64 payload is chunked, either the chunking should be
...
## Escape code safe UTF-8[¶](#escape-code-safe-utf-8 "Link to this heading")

This must be valid UTF-8 as per the spec in [**RFC 3629**](https://datatracker.ietf.org/doc/html/rfc3629.html). In addition, in order
...
## Identifier[¶](#identifier "Link to this heading")
